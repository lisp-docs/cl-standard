...
% CB trick found on stackoverflow here https://tex.stackexchange.com/questions/347030/force-evaluation-of-a-counter-in-endnote\
% to force evaluation of various things before getting passed to the outline command
\def\boutlineONE #1{%
	\edef\tmp{%
\noexpand\outline{2}{\chapno\bspace #1}}\tmp	
}


\def\boutlineTWO #1{%
	\edef\tmp{%
\noexpand\outline{2}{\chapno.\the\ssecn\bspace #1}}\tmp	
}

\def\boutlineTHREE #1{%
	\edef\tmp{%
\noexpand\outline{3}{\chapno.\the\ssecn.\the\sssecn\bspace #1}}\tmp	
}

\def\boutlineFOUR #1{%
	\edef\tmp{%
\noexpand\outline{4}{\chapno.\the\ssecn.\the\sssecn.\the\ssssecn\bspace #1}}\tmp	
}

\def\boutlineDICT {%
	\edef\tmp{%
\noexpand\outline{2}{\chapno.\the\ssecn\bspace Dictionary}}\tmp	
}


\def\startSection{\advance\ssecn 1\sssecn=0\ssssecn=0\sssssecn=0\ssssssecn=0}
\def\beginSection #1{\startSection \boutlineTWO{#1} \leftskip\normalleftskip\HeadII{\ #1}}
%Dictionary sections don't start with a banner line announcing their section name.
%CB looks like dictionaries are pretty much are always on level with 'startSection'
\def\includeDictionary #1{\startSection \boutlineDICT \toc2{\chapno.\the\ssecn\ {\chapline} Dictionary}
\input #1
\endSection}
    
\def\startSubsection{\advance\sssecn 1\ssssecn=0\sssssecn=0\ssssssecn=0}
\def\beginSubsection #1{\startSubsection   \boutlineTHREE{#1} \HeadIII{\ #1}}

\def\startsubsubsection{\advance\ssssecn 1\sssssecn=0\ssssssecn=0}
\def\beginsubsubsection #1{\startsubsubsection \boutlineFOUR{#1} \HeadIV{\ #1}}
...
\def\dobegincom
#1\ftype #2{\outline{3}{#1}\leftskip0pc\rightskip0pc plus10pc\bfsixteen
\baselineskip 16pt
